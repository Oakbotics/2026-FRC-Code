...
import frc.robot.ShootOnTheMove.HubShotCalculator;
import frc.robot.ShootOnTheMove.HubShotCalculator.Solution;
...
public class ShootOnMoveToHub extends Command {
  ...
  public void execute() {
    ...
    Translation2d hubPos = Constants.VisionAlignConstants.hubPosition();
  ...
  private void updateVisionOdometryIfHubTagVisible() {
    int tagID = (int) LimelightHelpers.getFiducialID(Constants.VisionAlignConstants.LIMELIGHT_NAME);
    if (tagID == -1 || !Constants.VisionAlignConstants.HUB_TAG_IDS.contains(tagID)) {
      return;
    }
    ...
    PoseEstimate est =
        (alliance.isPresent() && alliance.get() == Alliance.Blue)
            ? LimelightHelpers.getBotPoseEstimate_wpiBlue(Constants.VisionAlignConstants.LIMELIGHT_NAME)
            : LimelightHelpers.getBotPoseEstimate_wpiRed(Constants.VisionAlignConstants.LIMELIGHT_NAME);
